...
@section('content')
@include('laravelhtmldomparser::prizes.component.styles.mientrung') 
<div class="md:w-full"> 
    <h1 class="pagetitle">Kết quả sổ xố Miền Trung</h1>
    ...
    <div id="liveResultLottery">
        @if(count($prizesResult[0])>0)
            @include('laravelhtmldomparser::prizes.component.mientrung',['prizes'=>$prizesResult[0]]) 
        @endif
    </div> 
    @for($i=1;$i<count($prizesResult);$i++)
        @if(count($prizesResult[$i])>0)
            @include('laravelhtmldomparser::prizes.component.mientrung',['prizes'=>$prizesResult[$i]]) 
        @endif
    @endfor
</div>  
