...
@section('content')
@include('laravelhtmldomparser::prizes.component.styles.max3d') 
<div class="md:w-full">
    
    <h1 class="pagetitle">Kết Quả xổ số Max3D </h1>
    @include('laravelhtmldomparser::prizes.component.showPreviousPrizeVietlott')
    <div id="liveResultLottery">
        @include('laravelhtmldomparser::prizes.component.max3d',['prize'=>$prizes[0]])    
    </div> 
    @for($rowIndex=1; $rowIndex < count($prizes); $rowIndex++)
        @include('laravelhtmldomparser::prizes.component.max3d',['prize'=>$prizes[$rowIndex]])    
    @endfor
</div>  
